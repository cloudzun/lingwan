...
| 5 | **数据现状 + 测试样本** | 场景 B 的数据风险 | "有 5 份知识文档（见下，各多少章多少条）+ Q&A 验证集 6 条 + 库外问题 2 条；知识文档会更新，需重建索引。测试样本外部给定、**数据整段写进提示词**（含表头），原样使用、严禁重造" |
...
在启动信息里写清**来源目录、文件名与数量**，并把外部给定的测试数据**整段写进提示词**——知识文档只写清单，验证集与库外问题集连数据一起给：
...
> **为什么数据要整段写进提示词**：一是**防编数据**（不写，Agent 就会自己编一份"好做的"数据来跑，验收口径当场散掉）；二是**固定验收基准**（数据由数据包外部给定，写进去它才能在施工前落盘为 `tests/qa_pairs.csv`，与 qa 环节验收工具读的是同一份口径）；三是**原样照抄**（数据是验收口径的一部分，一个字都不能改；改了样本＝换了考卷，结论作废）。
...
- **八个信息块**：背景与角色、要做什么（含输入输出与字段契约）、技术选型（含理由）、数据现状、Q&A 验证集（**数据整段写进提示词**）、库外问题集（**数据整段写进提示词**）、验收口径（双指标 + 拒答）、约束。缺哪一块，回到本节 Step 1 的清单补。
...
- **知识文档只写清单**：`知识文档/` 5 份按章条清单写进【数据现状】，正文由 Agent 从数据包原位读取。
