...
- 技术栈：opencode + gstack 工程流程 + DeepSeek + Streamlit 验证面板。
...
对照实验一的方案框架，把场景 A 的相关结论摘出来，组装成一份完整的启动信息。数据包里的 `脚手架/启动提示词-模板.md` 已经把这份启动信息拆成 **9 个块**（其中【技术选型】【数据现状】【测试集】【环境与网络】有可直接采用的写法），一块都不许留白。**每块都给出确定的内容，不是留白让 Agent 猜**：

| # | 要包含的块 | 从方案框架哪里找 | 启动信息里怎么写 |
|---|------------|----------------|----------------|
| 1 | **背景与角色** | 实验一《解决方案框架》场景 A | "实验一《解决方案框架》把场景 A（客户诉求智能分类）定为快赢第一站"；写明 Agent 扮工程实现方、你负责判断与拍板；写明决策人、一线代表、数据归口 |
| 2 | **要做什么** | 场景 A 定义 | "做一个客户诉求五分类系统"；写成一句可验收的话，不要写"做一个智能客服系统" |
| 3 | **输入输出** | 场景 A 的"解决什么" | "输入一句客户诉求全文，输出类别：业务办理／电费与计量／故障报修与停电／投诉举报与用电安全／其他"，并写死 `confidence`／`reason`／`needs_human_review`／`status` 与落盘文件 |
...
| 8 | **环境与网络**（自动带入） | 固定的两条国内网络配置 | "pip 用清华镜像源；GitHub 访问走 ghfast.top 代理"——照抄即可，不用自己写 |
...
- [ ] 7 个自查问题你都能回答"能"
- [ ] 数据现状（无历史标注 → 零样本）写进去了
- [ ] **输入输出写清了**（输入是什么、输出哪些字段：类别 ＋ `confidence`／`reason`／`needs_human_review`／`status`）
- [ ] 测试样本写明了"外部给定、原样照抄"，并把 **25 条数据（含表头）连数据一起写进提示词**（Agent 施工前落盘为 `tests/classify_cases.csv`）
- [ ] **环境与网络那两条配置带上了**（pip 清华镜像源 ＋ GitHub ghfast 代理）
...
| **autoplan** | 暴露两可决策，等你拍板 | **真的拍板**（转人工阈值、兜底策略、要不要做面板滑块），写回 SPEC |
...
【背景】实验一《解决方案框架》把场景 A（客户诉求智能分类）定为快赢第一站、选型零样本
文本分类（客户诉求 → 专业条线类别）；本轮由 Delta 施工场景 A。
客户方决策人周主任、坐席班长小林、数字化部陈工，合规由省公司数字化与网络安全部把关。

【要做什么】做一个客户诉求五分类系统：把坐席每天要人工判断的「这条诉求该转到哪个专业
条线」自动化。拿不准走转人工、不硬归"其他"；只做五分类，不做全业务细分派单（完整 No-Go 交给
spec 环节产出）。

【输入输出】输入：一条客户诉求全文（如"我家车位想装个给电动车充电的表怎么申请"）。
输出：类别（业务办理／电费与计量／故障报修与停电／投诉举报与用电安全／其他）＋ confidence
＋ reason ＋ needs_human_review ＋ status。

【技术选型】一次大模型调用做零样本分类，配一条确定性兜底规则；不用 RAG/Agent
（分类一次调用即可定类别）。技术栈：Python ＋ DeepSeek ＋ Streamlit 验证面板。
...
【环境与网络】如需安装/克隆依赖，按以下配置执行：
1. pip 使用清华镜像源：先执行 pip config set global.index-url https://pypi.tuna.tsinghua.edu.cn/simple
2. GitHub 访问走代理 https://ghfast.top（克隆仓库时 URL 前缀加 https://ghfast.top/）
...
- **必备的块**：背景 / 要做什么 / **输入输出** / 技术选型 / 数据现状 / 测试样本（**数据整段写进提示词**，含表头 25 条）/ 验收口径 / 约束（边界一句话即可，**完整 No-Go 交给 spec 环节**）/ **环境与网络**（两条国内网络配置，照抄）。你漏了哪块，对照前面 9 块清单想——那是你写的时候没想到的。
...
### 测试样本（外部给定，数据整段写进启动提示词）

> **测试数据由讲师外部给定，放在数据包里（`评测集/qa_pairs.csv` 6 条 ＋ `评测集/qa_pairs_out_of_scope.csv` 2 条），提示词里连数据一起给，不要让 Agent 自己编数据。**

- **为什么样例必须外部给定**：验证集必须是"外部给定的"——由本手册与数据包提供固定数据，而不是让 Agent 自己出题自己考。数据固定，验收口径才固定。
- **文件口径**：`评测集/qa_pairs.csv` **6 条**（含表头 `question,expected_answer,expected_source`）、`评测集/qa_pairs_out_of_scope.csv` **2 条**（同表头）。
- **数据纪律（三条，改一条结论就作废）**：
  1. **原样照抄、严禁重造**——不改写、不删减、不扩写、不"挑好做的跑"、不让 Agent 另造一份；
  2. **把数据连数据一起写进启动提示词**——6 条 Q&A ＋ 2 条库外问题（含表头）原样写进去，让 Agent 施工前落盘为 `tests/qa_pairs.csv` 与 `tests/qa_pairs_out_of_scope.csv`，它就不用自己编数据；但数据一个字都不许改；
  3. **落盘由数据包提供**，不需要你手动搬运文件；验收工具直接读 `评测集/` 下的原始文件。
...
和场景 A 一样，一份好的启动信息要包含 7 项，另有 4 个固定块（政策文档、Q&A 验证集、库外问题集、环境与网络，由向导自动拼装，不占填写步数）。**场景 B 的内容不同**，对照实验一方案框架场景 B 填；提示词正文合计 11 个固定块，块序为：背景 → 要做什么 → 输入输出 → 技术选型 → 数据现状 → 政策文档 → Q&A 验证集 → 库外问题集 → 验收口径 → 约束 → 环境与网络。

| # | 要包含的信息 | 从方案框架哪里找 | 启动信息里怎么写 |
|---|------------|----------------|----------------|
| 1 | **背景与角色** | 你是谁、在给谁做 | "我是 FDE 团队的 Delta，施工场景 B：供电业务知识问答"；场景 B 的定位与选型来自实验一《解决方案框架》，上一轮（实验二）交付场景 A 并留下 AGENTS.md 与 retro.md |
| 2 | **要做什么** | 场景 B 定义 | "做一个供电业务知识问答系统，基于知识文档回答用电客户咨询" |
| 3 | **输入输出** | 场景 B 的"解决什么" | "输入一个业务问题，输出：答案文本 + evidence[] 来源链（文件名、条款号）；检索不到时 not_found" |
| 4 | **技术选型（含理由）** | 场景 B 选型 | "已选型 RAG，不用微调（知识文档常更新、需溯源）、不用纯关键词搜索（语义鸿沟）" |
| 5 | **数据现状 + 测试样本** | 场景 B 的数据风险 | "有 5 份知识文档（见下，各多少章多少条）+ Q&A 验证集 6 条 + 库外问题 2 条；知识文档会更新，需重建索引。测试样本外部给定、**数据块整段写进提示词**（含表头），原样使用、严禁重造" |
| 6 | **验收口径** | 场景 B 的成功衡量 | "答案准确率 ≥80% + 来源可追溯率 ≥90%（双指标，见下）；库外问题必须拒答" |
| 7 | **约束与边界** | 场景 B 的风险与应对 | "数据不出内网，只用 知识文档/ 与 评测集/；API key 走环境变量，不硬编码" |
| — | **固定块（不占填写步数）** | 由向导与数据包给定 | 【政策文档】5 份清单 / 【Q&A 验证集】6 条 / 【库外问题集】2 条 / 【环境与网络】pip 清华镜像源 + GitHub 走 ghfast.top 代理 |
| 8 | **流程要求** | 场景 B 的推进节奏 | 见本节 Step 3 代码块末尾一句："按 gstack 八环节流程执行 + 每环节产出文件 + 需要拍板时停下问我" |
...
**测试样本这一项，要按素材口径写对：数据由数据包外部给定，提示词里连数据一起给。** 把 6 条 Q&A 验证集与 2 条库外问题（含表头 `question,expected_answer,expected_source`）原样写进提示词，并写明"外部给定、原样使用、严禁重造"。理由有三条：

1. `脚手架/AGENTS.md` 把"测试样本外部给定，不得重造"列为不可协商的项目级硬约束；
2. 数据写进提示词，Agent 才能在施工前落盘成固定测试集——不写，它就会自己编一份"好做的"数据；
3. 抄写过程本身就是改写风险——8 条数据里改错一个字，验收口径就变了。

> **注意：** 这份数据是"外部给定"的测试集，**原样照抄，不要改、不要让 Agent 重新造一份**。它的作用是固定验收口径——6 条问答验证集 ＋ 2 条库外问题、答案确定，Agent 才能公平地被检验。
>
> **知识文档不同：只写清单，不抄正文。** 知识库（数据包 `知识文档/`）是"知识来源"，不是验收样本；提示词里按章条清单写清 5 份文档与各多少章多少条即可，正文由 Agent 从数据包原位读取。

### Step 2：组装启动提示词——先自己想，再看提示

> **不要急着看参考答案。** 写启动提示词的本质，是"**如果换一个人接手这个项目，他需要知道什么，才能不重开讨论、直接开干？**"

**先尝试自己组装。写的时候，用这 8 个问题自我审查：**

| # | 自查问题 | 为什么问这个 |
|---|---------|------------|
| 1 | 换一个不了解项目的人读，他能知道**我在为谁、做什么**吗？ | 缺了背景，Agent 会脑补客户和场景 |
| 2 | 他能不能知道**选型从哪来**（场景 B 的定位与 RAG 选型出自实验一《解决方案框架》）？ | 缺了来源，Agent 会重开一轮选型讨论 |
| 3 | 他能不能知道**输入是什么、输出是什么**（含来源链与 not_found）？ | 缺了 I/O，Agent 会发明错误接口 |
| 4 | 他能不能知道**"为什么选 RAG 而不是微调/关键词搜索"**？ | 缺了选型理由，Agent 可能问你"要不要微调" |
| 5 | 他能不能知道**数据是什么情况**（文档在哪、多少份、验证集在哪）？ | 缺了数据现状，Agent 可能凭记忆回答或找不到测试集 |
| 6 | 他能不能知道**"做出来怎么算成功"**？ | 缺了验收口径（尤其"来源可追溯"），Agent 会自己发明一套 |
| 7 | 他能不能知道**有什么红线**？ | 缺了约束，Agent 可能不重视来源引用 |
| 8 | 他能不能知道**"按什么流程跑、产出什么"**？ | 缺了流程要求，Agent 不会主动走 gstack 八环节 |
...
- [ ] 8 个自查问题你都能回答"能"
- [ ] 背景块点明了"场景 B 的定位与选型来自实验一《解决方案框架》"，并承接上一轮（实验二）的 AGENTS.md 与 retro.md
- [ ] 【输入输出】单独成块（输入一个业务问题；输出答案文本 + evidence[] 来源链；检索不到时 not_found）
- [ ] 数据现状写进去了（5 份知识文档在数据包 `知识文档/`，各多少章多少条；Q&A 6 条＋库外 2 条）
- [ ] 测试样本写明了"外部给定、原样照抄"，并把 **6 条 Q&A ＋ 2 条库外问题（含表头）连数据一起写进提示词**（Agent 施工前落盘为 `tests/qa_pairs.csv` 与 `tests/qa_pairs_out_of_scope.csv`）
- [ ] 验收口径（答案 ≥80% + 来源可追溯 ≥90%，库外问题必须拒答）写进去了
- [ ] 【环境与网络】固定块带上了（pip 清华镜像源 ＋ GitHub 走 ghfast.top 代理；这两个块不用你填，核对没漏即可）
- [ ] 明确写了"按 gstack 八环节流程执行 + 每环节产出文件 + 需要拍板时停下问我"
- [ ] 你和组内对齐了这份提示词（Delta 主导，Echo 队友审查）

> 写完、组内对齐后，可以对照**附录 D：启动提示词参考答案**——看你自己写的，相比参考答案漏了什么、多了什么。

### Step 3：把固定块按序拼进提示词（含政策文档、两套测试数据与环境与网络）

提示词正文合计 11 个固定块，块序固定为：**背景 → 要做什么 → 输入输出 → 技术选型 → 数据现状 → 政策文档 → Q&A 验证集 → 库外问题集 → 验收口径 → 约束 → 环境与网络**，末尾再补一句流程要求。

其中【数据现状】由你填（写清来源目录、文件名与数量）；**【政策文档】【Q&A 验证集】【库外问题集】【环境与网络】是固定块**——政策文档只写清单、测试数据连数据一起给、环境与网络照抄下面的网络配置，都不占你的填写步数（向导里自动拼装）：
...
【政策文档】知识库在 知识文档/ 目录下，共 5 份（混合格式）：供电营业规则（.md，8 章 32 条）、
电价与电费（.md，6 章 25 条）、业扩报装（.md，7 章 26 条）、供电服务规范（.docx，6 章 26 条）、
分布式光伏并网（.pdf，6 章 21 条）。.docx 与 .pdf 需先解析成可检索文本再入库；
正文由你从数据包原位读取，不得自行改动或编造。

【Q&A 验证集】请把以下测试数据写入 tests/qa_pairs.csv（含表头 question,expected_answer,expected_source），施工时用它作为自测集；数据不得自行改动或重新生成：
question,expected_answer,expected_source
居民低压新装申请需要提交哪些材料,需要提交申请人身份证明、用电地址房屋权属证明（或租赁合同及出租方同意）、委托书、用电设备清单及容量说明等材料（教学示意值）；受理后按约定时限答复供电方案并安排装表接电,岭湾电力业扩报装办理指南（教学合成版）.md 第十二条
居民阶梯电价的第二档如何划分并加价,居民阶梯电价按客户月用电量划分三档：第二档为月用电量超过 180 千瓦时、不超过 400 千瓦时的部分，在基础电价基础上每千瓦时加价 0.05 元（教学示意值），具体标准以现行规定为准,岭湾电力电价与电费计收说明（教学合成版）.md 第十一条
计划停电一般提前几天通知客户,因供电设施计划检修需要停电的，应当提前 7 日通知受影响的用电客户（教学示意值）；计划停电时间变更时应当重新通知,岭湾电力供电服务规范与服务承诺（教学合成版）.docx 第十四条
电费出现差错后客户在多长时间内可以申请退补,客户认为电费计算有误的，可以自收到电费账单之日起 30 日内提出电费差错核查申请（教学示意值）；核查与退补按第二十二条规定办理,岭湾电力电价与电费计收说明（教学合成版）.md 第二十一条
居民分布式光伏并网申请需要满足什么条件,应当同时具备下列条件：申请人为用电地址产权人或经产权人书面同意的使用人、项目所在建筑物具备安装光伏组件的结构安全条件、项目已安装电能计量装置并具备并网计量条件、项目所在配电台区具备接纳能力（教学示意值）,岭湾电力分布式光伏并网服务指引（教学合成版）.pdf 第五条
供电抢修人员到达现场有没有时限要求分别是多少,抢修人员应自接到报修之时起按时限到达现场：城区不超过 45 分钟、农村不超过 90 分钟、边远地区不超过 120 分钟（教学示意值）；因不可抗力不能按时到达的应当及时告知预计到达时间,岭湾电力供电服务规范与服务承诺（教学合成版）.docx 第十一条

【库外问题集】请把以下测试数据写入 tests/qa_pairs_out_of_scope.csv（含表头 question,expected_answer,expected_source），施工时用它作为自测集；数据不得自行改动或重新生成：
question,expected_answer,expected_source
我家燃气灶点不着火了该找谁修,知识库中未找到，建议咨询人工,不在知识库范围内
小区物业把水费涨了有没有依据,知识库中未找到，建议咨询人工,不在知识库范围内
...
> **样本纪律（改一条结论就作废）**：数据由数据包外部给定，必须**原样照抄、严禁重造**——不改写、不删减、不扩写、不"挑好做的跑"、不让 Agent 另造一份；数据块整段原样写进提示词，让 Agent 施工前落盘为 `tests/qa_pairs.csv`（6 条）与 `tests/qa_pairs_out_of_scope.csv`（2 条）。字段与数据包实物一致（表头 `question,expected_answer,expected_source`）；改一个字＝换了考卷，结论作废。
> **为什么数据要整段写进提示词**：一是**防编数据**（不写，Agent 就会自己编一份"好做的"数据来跑，验收口径当场散掉）；二是**固定验收基准**（数据由数据包外部给定，写进去它才能在施工前落盘，与 qa 环节验收工具读的是同一份口径）；三是**原样照抄**（数据是验收口径的一部分）。
> **知识文档只写清单**：知识库是"知识来源"、不是"验收样本"；提示词里按章条清单写清 5 份文档与各多少章多少条即可，正文由 Agent 从数据包 `知识文档/` 原位读取。
...
> **环境与网络为什么是固定块**：课堂环境统一配好 pip 清华镜像源与 GitHub 代理（ghfast.top），照抄即可，不必自己判断——它是环境的既定配置，不是你要设计的方案。
...
【在这里粘贴你写好的启动提示词，包括 11 个固定块：背景 / 要做什么 / 输入输出 / 技术选型 /
数据现状 / 政策文档 / Q&A 验证集 / 库外问题集 / 验收口径 / 约束 / 环境与网络】
...
- [ ] 写了高质量的 office-hours 启动提示词：11 个固定块齐全（背景承接实验一《解决方案框架》+ 要做什么 + **输入输出** + 技术选型 + 数据现状 + 政策文档清单 + **Q&A 验证集 6 条与库外 2 条连数据一起写进提示词** + 双指标验收口径 + 约束 + **环境与网络**）
- [ ] 【输入输出】独立成块（输入什么、输出答案文本 + evidence[] 来源链、检索不到返回 not_found）
- [ ] Q&A 验证集 6 条与库外 2 条**一条没改**（文本与 `评测集/qa_pairs.csv`、`评测集/qa_pairs_out_of_scope.csv` 一致）
- [ ] 【环境与网络】固定块带上了（pip 清华镜像源 ＋ GitHub 走 ghfast.top 代理）
- [ ] Agent 按 gstack 八环节自动跑完，每环节有产出文件
...
【背景】实验一《解决方案框架》把场景 B（供电业务知识问答）定为第二个快赢场景、
选型 RAG；上一轮（实验二）已交付场景 A（诉求分类），并留下 AGENTS.md 与 retro.md 作为
交接经验。本轮由 Delta 施工场景 B。

【要做什么】做一个供电业务知识问答系统：用电客户或坐席输入业务问题，系统从知识文档
检索证据再生成回答，并标注来源（哪份文件、哪一条）。

【输入输出】输入：一个客户业务问题（如"居民阶梯电价的第二档如何划分并加价"）。
输出：答案文本 ＋ evidence[] 来源链（文件名 ＋ 条款号）；检索不到时 not_found。
...
- **十一个块**：背景与角色（承接实验一《解决方案框架》）、要做什么、**输入输出**（输入什么／输出什么字段）、技术选型（含理由）、数据现状、政策文档（5 份清单）、Q&A 验证集（**数据整段写进提示词**）、库外问题集（**数据整段写进提示词**）、验收口径（双指标 ＋ 拒答）、约束与边界（红线一句话）、**环境与网络**（pip 清华镜像源 ＋ GitHub ghfast 代理）。
- **两处"防翻车"信息**：1）无证据必须返回 `not_found`，不得编造条款号或数值（防编造，底线）；2）回答中的数值一律标注"（教学示意值）"（防把教学数字当真实标准）。
- **测试数据连数据一起给了**：【Q&A 验证集】的 6 条与【库外问题集】的 2 条（含表头 `question,expected_answer,expected_source`）**原样写进提示词**，并注明"外部给定、原样使用、严禁重造"——Agent 施工前落盘为 `tests/qa_pairs.csv` 与 `tests/qa_pairs_out_of_scope.csv`，不必自己编数据。
- **知识文档只写清单**：`知识文档/` 5 份按章条清单写进【政策文档】，正文由 Agent 从数据包原位读取。
- **输入输出独立成块**：出了【要做什么】就说清"输入什么、输出什么、检索不到返回什么"；字段口径固定为 answer + evidence[]（document/article/excerpt）+ evidence_sufficient + status，字段明细交 SPEC。
- **边界一句话说清不做什么即可；完整 No-Go 交给 spec 环节**。
- **环境与网络是固定块**：pip 清华镜像源 ＋ GitHub 走 ghfast.top 代理，照抄即可，不是要设计的方案。
...
带一个 Streamlit 验证面板，验收时三路都要走通，且**敏感件零漏判**。
...
一份好的启动信息由 **10 个信息块**组成，**场景 C 的内容不同**，对照实验一方案框架场景 C 逐块填。向导里是 7 个填写步（前 7 块由学员写），其余 3 块是固定块（两个数据块连数据自动带入、环境与网络照抄）：

| 步 | 信息块 | 从方案框架哪里找 | 启动信息里怎么写 |
|---|------|----------------|----------------|
| 1 | **背景与角色** | 你是谁、在给谁做 | "实验一《解决方案框架》把场景 C 定为第三站、选型固定路由工作流；场景 A／B 已完成，其 AGENTS.md 与 retro.md 作为交接经验；本轮由 Delta 施工场景 C。" |
| 2 | **要做什么** | 场景 C 定义 | "做一个工单分级路由工作流，按咨询/跨专业协同/敏感三档分流处理" |
| 3 | **输入输出**（独立成块） | 场景 C 的"解决什么" | "输入一份客户工单，输出：分级 + 对应处理分支的动作" |
...
| 6 | **部门职责库**（固定块） | 数据包 `职责库/departments.csv` | 连数据（含表头，6 行）整段照抄进提示词，指明落盘 `data/departments.csv` |
| 7 | **工单样本**（固定块） | 数据包 `评测集/workorders.csv` | 连数据（含表头 `id,text,label`，6 份）整段照抄进提示词，指明落盘 `tests/workorders.csv` |
| 8 | **验收口径** | 场景 C 的成功衡量 | "路由正确率 ≥90% + 敏感件零漏判（见下）" |
| 9 | **约束与边界** | 场景 C 的风险与应对 | "敏感诉求宁可多升级不可漏、命中必须真暂停；API key 走环境变量" |
| 10 | **环境与网络**（固定块） | 课程环境约定 | 照抄固定两条：pip 清华镜像源、GitHub 走 `https://ghfast.top` 代理 |

> **块序照素材固定，10 块一块不省：** `【背景】→【要做什么】→【输入输出】→【技术选型】→【数据现状】→【部门职责库】→【工单样本】→【验收口径】→【约束】→【环境与网络】`。
> **两处与上一版不同，别写错位置：** 1）**【输入输出】独立成块**，排在【要做什么】之后、【技术选型】之前，**不再并进【要做什么】**；2）**【环境与网络】是固定块**，排在【约束】之后、末尾流程句之前，照抄不改（装依赖的网络配置不在别处重复写）。

> **第 5、8 块是最容易漏的，写不好 Agent 会卡住或走错路线：**
>
...
- [ ] 你自己独立写出了启动提示词（先别翻附录）
- [ ] 7 个自查问题你都能回答"能"
- [ ] **10 个信息块**全部齐备（背景／要做什么／输入输出／技术选型／数据现状／部门职责库／工单样本／验收口径／约束／环境与网络）
- [ ] **【输入输出】独立成块**（排在【要做什么】之后、【技术选型】之前），**没有并进【要做什么】**
- [ ] 数据现状写进去了，且**工单样本与专业职责库连数据一起**（含表头：工单 6 份／职责库 6 行）整段写进提示词，一个字没改；敏感清单 8 条点明为敏感档判定依据
- [ ] 验收口径（路由正确 ≥90% + 敏感件零漏判）写进去了
- [ ] **【环境与网络】固定块照抄**（pip 清华镜像源 ＋ GitHub 走 `https://ghfast.top` 两条），排在【约束】之后
- [ ] 明确写了"按 gstack 八环节流程执行 + 每环节产出文件 + 需要拍板时停下问我"
- [ ] 你和组内对齐了这份提示词（Delta 主导，Echo 队友审查）
...
> **剩下两块的归属说清楚：**【输入输出】块由学员在向导第 3 步自己写（**独立成块**，不并进【要做什么】）；**【环境与网络】是固定块**——把下面两行照抄进提示词末尾（排在【约束】之后、流程句之前）即可，装依赖时按它执行：
...
【在这里粘贴你写好的启动提示词，包括：背景 / 要做什么 / 输入输出 /
技术选型 / 数据现状 / 验收口径 / 约束 / 环境与网络】
...
- [ ] 写了高质量的 office-hours 启动提示词（**10 个信息块齐备且块序正确**：背景／要做什么／输入输出／技术选型／数据现状／部门职责库／工单样本／验收口径／约束／环境与网络；含数据现状 + 工单样本与专业职责库连数据 ＋ 双指标验收口径）
- [ ] 【输入输出】**独立成块**（未并进【要做什么】）；【环境与网络】固定块照抄（pip 清华源 ＋ GitHub 代理）
...
【背景】实验一《解决方案框架》把场景 C（供电服务工单智能分级路由）定位为第三站、选型为固定路由
工作流（Routing Workflow）；场景 A（客户诉求分类）与场景 B（知识库检索问答）已完成，其 AGENTS.md
与 retro.md 作为交接经验。本轮由 Delta 施工场景 C。
...
【输入输出】输入：一份客户工单文本。
输出：分级结果 + 对应分支动作（咨询→自动答复；跨专业协同→查专业职责库；敏感→转人工真暂停）；
字段契约（primary_department／collaborating_departments／submitted／interrupted）明细交 SPEC。
...
- **必备信息块**：**共 10 块**——7 个短信息块（背景／要做什么／输入输出／技术选型（含理由）／数据现状／验收口径／约束，每块不超过 3 行）＋ 2 个数据块（【部门职责库】【工单样本】，连数据整段附在【数据现状】之后）＋ 1 个固定块（【环境与网络】，照抄两条网络配置，排在【约束】之后）。**块序固定：**`【背景】→【要做什么】→【输入输出】→【技术选型】→【数据现状】→【部门职责库】→【工单样本】→【验收口径】→【约束】→【环境与网络】`。
- **【输入输出】是独立一块**（不是并进【要做什么】的附注）：只写"输入一份客户工单；输出分级结果 ＋ 对应分支动作"，字段契约明细交 SPEC。
