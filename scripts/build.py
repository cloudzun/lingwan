...
def build_handbook(pages):
    ...
    ov = (f'<div class="ov-hero"><div class="tag">学员版 · 实验手册</div><h1>{BRAND} · 实验手册总览</h1>'
          ...
          f'<p><b>课前环境与网络（国内）：</b>'
          f'① <b>Python 包下载</b>建议用清华镜像源——<code>pip config set global.index-url https://pypi.tuna.tsinghua.edu.cn/simple</code>；'
          f'② <b>GitHub 访问</b>如不通畅用加速代理 <code>https://ghfast.top</code>（克隆时 URL 前缀加 <code>https://ghfast.top/</code>）——'
